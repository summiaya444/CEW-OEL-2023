...
static size_t callback(void *data,size_t size,size_t n_data,void *userp){
    ...
    if (mem->memory==NULL){
        mem->memory=malloc(1); //firstly initialize to an empty string to avoid memory leakage
        if (mem->memory==NULL){
    ...
    mem->memory=repeat;
    memcpy(&(mem->memory[mem->size]),data,realsize); //destination address which is to be the null char,data to be copied,realsize no of bytes
    mem->size+=realsize;
...
int fetch_data(const char *api_key,const char *city){
    ...
    sprintf(api_url, "http://api.openweathermap.org/data/2.5/weather?q=%s&appid=%s", city, api_key);
