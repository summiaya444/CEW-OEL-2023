        ...
        switch (server_response) {
		    ...
		    double windspeed = get_real_value(wind, "speed");
                    ...
                    if (temperature > temperature_high) {
                        ...
                        printf("\n❄ Brrr! Temperature dipping below %.2f °C ❄\n", temperature);
...
void send_email(const char *subject, const char *body) {
    ...
    char command[512];
    snprintf(command, sizeof(command), "echo \"%s\" | mutt -s \"%s\" irzasalahuddin27@gmail.com", body, subject);
...
void send_email(const char *subject, const char *body, const char *attachment) {
    // Construct the Shell command
    char command[512];
    snprintf(command, sizeof(command), "./send_email.sh \"%s\" \"%s\" \"%s\"", subject, body, attachment);
...
int main() {
    ...
    if (run_time <= max_runs) {
        ...
        if (run_time == max_runs) {
			...
			fprintf(weekly_report_file, "❄ Cool temperatures observed. Bundle up!\n");
		    } else {
            ...
            send_email(subject,body,weekly_report);
            free(body);
        }
        set_run_time();
